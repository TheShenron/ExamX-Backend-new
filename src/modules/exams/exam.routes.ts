...
router.post("/", verifyToken, upload.single('examZipFile'), validateReq({ body: createExamSchema }), allowRoles(ADMIN), createExam);
router.get("/", verifyToken, allowRoles(ADMIN, HR), getAllExams);
router.put("/:id", verifyToken, upload.single('examZipFile'), validateReq({ body: updateExamSchema }), allowRoles(ADMIN), updateExam);
